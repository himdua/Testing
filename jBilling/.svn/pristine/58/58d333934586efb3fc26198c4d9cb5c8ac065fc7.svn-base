...
public class PaymentsPage {
	...
	private ElementField TB_PAYMENTAMOUNT;
	...
	/**
	 * This method is used for click on the Make Payment button
	 *
	 * @author Shanky
	 * @return
	 * @throws Exception
	 */
	protected PaymentsPage clickOnMakePaymentBtn() throws Exception {
		GlobalController.brw.clickLinkText(this.LT_MAKEPAYMENT);
		return GlobalController.brw.initElements(PaymentsPage.class);
	}

	/**
	 * This method will make payment with Different Types
	 *
	 * @author Shanky
	 * @param payment
	 ...
	 * @throws Exception
	 */
	public PaymentsPage MakePayment(MakePayment payment, String testDataSetName, String category, String PaymentMethod) throws Exception {
		String chequeHolderName = TestData.read("PaymentsPage.xml", testDataSetName, "chequeHolderName", category);
		String chequeNumber = TestData.read("PaymentsPage.xml", testDataSetName, "chequeNumber", category);
		String chequeDate = TestData.read("PaymentsPage.xml", testDataSetName, "chequeDate", category);
		String paymentAmount = TestData.read("PaymentsPage.xml", testDataSetName, "paymentAmount", category);
		...
		case MODIFYPAYMENT:
			...
			this.setPaymentOrder(paymentOrder);
			this.clickReviewPayment();
			this.clickOnMakePaymentBtn();
			break;

		case UNCHANGEDPAYEMENT:
